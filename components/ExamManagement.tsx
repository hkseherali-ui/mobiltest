...
const ExamManagement: React.FC = () => {
  ...
  const exportPDF = (exam: Exam) => {
    ...
    doc.setFont("helvetica", "normal"); // Bold sorunu çözüldü
        ...
        {editingExam.questions.map((q, idx) => (
          <div key={q.id} className="bg-white p-6 rounded-[2.5rem] border border-gray-100 space-y-4 relative shadow-sm">
             ...
             <div className="grid grid-cols-2 gap-2">
                <label className="text-center py-2 bg-gray-100 rounded-xl text-[9px] font-black cursor-pointer">🖼️ GÖRSEL EKLE
             ...
             <div className="space-y-2">
                {q.options.map((opt, oIdx) => (
                  <div key={oIdx} className="flex gap-2 items-center bg-gray-50 p-2 rounded-xl">
                    ...
                    <label className="p-2 bg-white rounded-lg cursor-pointer text-xs">📷
        ...
        {showPool && (
          <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-md rounded-[3rem] p-6 max-h-[70vh] flex flex-col shadow-2xl">
               <div className="flex justify-between items-center mb-4"><h3 className="font-black text-xs uppercase">Soru Havuzu</h3><button onClick={() => setShowPool(false)} className="text-2xl">×</button></div>
    ...
    <div className="p-4 space-y-4">
      ...
      {exams.map(e => (
        <div key={e.id} className="bg-white p-5 rounded-[2.5rem] border border-gray-50 flex justify-between items-center shadow-sm">
          <div className="truncate flex-1 text-left"><h4 className="font-black text-xs uppercase">{e.title}</h4><span className="text-[8px] text-indigo-400 font-bold">Sınıflar: {e.targetClasses?.join(', ')}</span></div>
          <div className="flex gap-2">
            ...
            <button onClick={async () => { if(confirm("Silinsin mi?")) { await dbService.deleteExam(e.id); refresh(); } }} className="w-10 h-10 bg-red-50 text-red-600 rounded-xl flex items-center justify-center text-xs">🗑️</button>
          </div>
        </div>
      ))}
    </div>
